...
while running:
    ...
    if elapsed_time >= game_time_limit:
        ...
        continue
    #### 7 
    if fly_visible and fly_start_time is not None:
        time_since_appearance = pygame.time.get_ticks() - fly_start_time
        if time_since_appearance >= fly_duration:
            fly_visible = False
            fly_position = None
            fly_start_time = None
            pygame.time.set_timer(pygame.USEREVENT, random.randint(0, 20))  # Schedule next appearance
    ##### 7 
